...
class DeduplicationHandler extends BufferHandler
{
    ...
    public function __construct(HandlerInterface $handler, ?string $deduplicationStore = null, $deduplicationLevel = Logger::ERROR, int $time = 60, bool $bubble = true)
    {
        ...
        $this->deduplicationStore = $deduplicationStore === null ? sys_get_temp_dir() . '/monolog-dedup-' . \Phabel\Target\Php80\Polyfill::substr(md5(__FILE__), 0, 20) . '.log' : $deduplicationStore;
    ...
    private function collectLogs() : void
    {
        ...
        while (!feof($handle)) {
            $log = fgets($handle);
            if ($log && \Phabel\Target\Php80\Polyfill::substr($log, 0, 10) >= $timestampValidity) {
                $validLogs[] = $log;
            }
        }
        ftruncate($handle, 0);

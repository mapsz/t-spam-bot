...
final class Utils
{
    ...
    public static function getClass(object $object) : string
    {
        ...
        if (false === ($parent = \get_parent_class($class))) {
            return \Phabel\Target\Php80\Polyfill::substr($class, 0, $pos + 10);
        }
    ...
    public static function substr(string $string, int $start, ?int $length = null) : string
    {
        ...
        return \Phabel\Target\Php80\Polyfill::substr($string, $start, null === $length ? strlen($string) : $length);
    }
    /**
    ...
    {
        $prefix = '';
        if ('file://' === \Phabel\Target\Php80\Polyfill::substr($streamUrl, 0, 7)) {
            $streamUrl = \Phabel\Target\Php80\Polyfill::substr($streamUrl, 7);
            $prefix = 'file://';
        }
        ...
        // already absolute
        if (\Phabel\Target\Php80\Polyfill::substr($streamUrl, 0, 1) === '/' || \Phabel\Target\Php80\Polyfill::substr($streamUrl, 1, 1) === ':' || \Phabel\Target\Php80\Polyfill::substr($streamUrl, 0, 2) === '\\\\') {
    ...
    */
    ...
    public static function pcreLastErrorMessage(int $code) : string
    {
        ...
        $constants = array_filter($constants, function ($key) {
            return \Phabel\Target\Php80\Polyfill::substr($key, -6) == '_ERROR';
